...
- **T2 — Scale vs. interactivity (implemented):** a Curated 15 / All scope toggle reads the full ~600-symbol universe from the same fetched response (zero extra requests); hand-rolled windowed table rendering (bounded DOM, spacers, edge auto-scroll), prebuilt search index for filtering at scale, history bounded to curated∪pinned, reorder bounded to curated∪pinned.
- **T3 — Instant feel vs. durable order (implemented):** versioned `order.v2` record written through an injectable `LocksPort` (`navigator.locks` in production, v1 migrated and kept for rollback), strictly-newer `updatedAt` cross-tab adoption, rejected writes resync the losing tab; gestures stay optimistic.
...
- 117 tests / 117 passing (66 server + 50 browser + 1 E2E via `remix test`), `tsc --noEmit` clean.
- Every AC-1..103 maps to at least one test titled with its AC id; unit tests use injected `KVStore`/clock doubles (deterministic cross-tab race simulation, incl. the documented T1 overdraw race), router tests pin the no-error-page cold start, Chromium component tests cover interaction and rendering.
...
- Spec: `specs/crypto-dashboard.spec.md` — AC-1..75 approved at the gate; AC-76..103 added via the spec's Amendments protocol (review findings, the view toggle, the T2/T3 implementation, and QA/review hardening incl. the single E2E hydration criterion, AC-100).
- Design: `docs/design/crypto-dashboard.md`; decisions: `docs/adr/0001`–`0007` (Remix 3 stack, tokens-as-CSS-asset, native DnD + keyboard path, remix-native test stack, server-chrome/client-hydration split).
